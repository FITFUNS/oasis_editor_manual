...
In order to create scripts for OasisW, it is vital that you know how to access and use your browser's development tools.
...
The navigator lists all of the scripts currently running in the active tab, including any OasisW scripts that you have written. Find one of your scripts in the navigator and select it to open the source code. You are now able to set breakpoints and debug.
...
Note that when a running app is paused at a breakpoint in the debugger, other browser windows/tabs used to launch that app (containing the OasisW Code Editor or Editor etc.) might also be paused.
