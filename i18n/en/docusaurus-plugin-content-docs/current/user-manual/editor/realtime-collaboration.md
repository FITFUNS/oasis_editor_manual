...
Real-time collaboration is at the heart of the OasisW Editor. This brings a number of benefits:
...
In the bottom left corner of the [Viewport](/user-manual/editor/interface/viewport) (next to the CHAT button), you will find the Presence Bar.
...
You can toggle browser notifications for chat messages in the [Settings](/user-manual/editor/interface/settings/editor#settings).
...
Each user in the scene is represented in the [Viewport](/user-manual/editor/interface/viewport) by a colored, wireframe camera frustum.
...
It can be useful to know what entities other users are selecting and potentially editing. The [Hierarchy](/user-manual/editor/interface/hierarchy) displays square indicators to the right of entities selected by other users (shaded according to their user color).

![Selection Indicators](/img/user-manual/editor/realtime-collaboration/selection-indicators.gif)

Whenever an entity with a 3D model is selected by any user, its outline will be rendered in the [Viewport](/user-manual/editor/interface/viewport).
