...
This document is aimed at people who are complete beginners to web programming and describes at high level how the different parts of a web application combine to serve OasisW applications, or other web pages to a user's browser.
...
When writing OasisW applications, no code you write in OasisW runs server-side and we don't provide any server-side code for your projects.
...
Or in the most complex case, you can write a full 3D WebGL game using OasisW. Because everything you write using OasisW is client-side javascript.

### Static Content

Some parts of your web application are not dynamic and do not need to change. For example, images, audio files, text files and in the case of OasisW applications 3D models and textures. You can think of this a bit like loading files of a local disk, except that of course it comes over the internet so it's a bit slower. Serving static content is done by a web server, very much like server-side code and in some cases it will be the same machine. As there is no interactivity, there is no processing done to fulfill the request. The web server just sends back the requested file.

The important thing about static content is making sure it downloads to the client as fast as possible.

## Hosting OasisW Applications

When you build a OasisW application your are writing client-side javascript code and creating assets which are static content. In OasisW projects you will have no server-side code at all.

When you publish using OasisW we handle the server-side by setting up a URL where your application is served from sending back the HTML page that starts your client-side application. When the HTML page loads it runs the client-side javascript of your OasisW application this will load all your assets from our static content servers.

We have designed the OasisW hosting to be optimized for serving your OasisW applications to end users throughout the world as quickly as possible. This is why the easiest way to deploy a OasisW application is by clicking the publish button and sending out the `playcanv.as` URL. However, in some cases you may be required to host your own application.

## Self-hosting OasisW Applications

We have a more [detailed page][1] about self-hosting a OasisW application. But in summary, to self-host you need to provide the three parts of the web stack described above.
...
As described in **Server** you will be serving all your OasisW content including the `index.html` file from your web server as static content.
...
Not all servers have content types setup for common formats used in OasisW and can give errors when attempting to load the application from the server.

If you do get errors or warnings about assets not being served correctly in the browser, you may need to add these types.

Here's a list of content types used in OasisW applications:
...
The client-side code is everything you have written as part of your OasisW application. When the browser loads the `index.html` file it will start your OasisW application and run all the client-side code that you have written.
